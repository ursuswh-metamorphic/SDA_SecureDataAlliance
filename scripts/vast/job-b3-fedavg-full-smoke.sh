...
FEDE="$ROOT/FedE"
OUT="$ROOT/artifacts/training/b3_fedavg_full_smoke_seed13"
EVAL_OUT="$ROOT/artifacts/eval/b3_fedavg_full_smoke_val_seed13"
WORK="$OUT/work"
LOG="$OUT/job.log"
DONE="$OUT/job.done"
...
{
  echo "run_name=b3_fedavg_full_smoke_seed13"
  echo "protocol=5 company-exclusive clients; weighted FedAvg; full fine-tune"
...
/workspace/sda-venv/bin/python -u "$FEDE/eval_clean.py" \
  ...
  --top-k 100 \
  --seed 13 \
  --run-name b3_fedavg_full_smoke_val_seed13 \
  --output "$EVAL_OUT"
