...
FEDE="$ROOT/FedE"
OUT="$ROOT/artifacts/training/b3_fedavg_full_pilot_seed13"
EVAL_OUT="$ROOT/artifacts/eval/b3_fedavg_full_pilot_val_seed13"
WORK="$OUT/work"
LOG="$OUT/job.log"
DONE="$OUT/job.done"
...
{
  echo "run_name=b3_fedavg_full_pilot_seed13"
  echo "protocol=5 company-exclusive clients; datavol-weighted FedAvg; full fine-tune"
...
exec >>"$LOG" 2>&1
echo "[B3] started at $(date -u +%FT%TZ)"
...
/workspace/sda-venv/bin/python -u "$FEDE/eval_clean.py" \
  ...
  --top-k 100 \
  --seed 13 \
  --run-name b3_fedavg_full_pilot_val_seed13 \
  --output "$EVAL_OUT"

echo "0" >"$DONE"
echo "[B3] completed at $(date -u +%FT%TZ)"
