...
def choose_query_transform_mode(question: str) -> str:
    q = question.lower()

    # Câu hỏi phức hợp
    if any(kw in q for kw in [" and ", " compare ", " respectively", " both ", " list and"]):
        return "subquery_zeroshot"

    # Câu hỏi “how/why/explain”
    if any(q.startswith(kw) for kw in ["how ", "why ", "explain ", "in what way"]):
        return "stepback_zeroshot"

    # Default: fact-based
    return "hyde_zeroshot"