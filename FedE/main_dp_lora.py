...
def train_client(client_id, global_state, client_dataset, sigma, clip_norm=1.0):
    ...
    for step in range(num_steps):
        ...
        if step % max(1, num_steps//3) == 0:
            print(f'  [Client {client_id}] step {step}/{num_steps}, loss={loss.item():.4f}, grad_norm={grad_norm:.4f}')
...
def aggregate_lora(global_state, client_states, sigma, clip_norm=1.0):
    """Average LoRA params with server-level DP noise."""
    K = len(client_states)
    ...
    for key in lora_keys:
        ...
        noise = torch.randn_like(avg) * noise_std
        avg_state[key] = avg + noise
...
for round_num in range(1, NUM_ROUNDS + 1):
    ...
    for cid in range(NUM_CLIENTS):
        cs = train_client(cid, global_state, client_data[cid],
                         sigma=calibrated_sigma, clip_norm=1.0)
